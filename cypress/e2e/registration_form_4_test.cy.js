...
describe('Input fields', () => {
    ...
    it('Username tooltip is visible', () => {
        cy.get('#username').type('{enter}')
        cy.get('h2').contains('Password').click()
        cy.get('.input').should('have.attr', 'title').should('contain', 'Please add username')
    ...
    it('Username should support only letters and numbers', () => {
        ...
        cy.get('#username').should('have.attr', 'pattern', '[a-zA-Z0-9]+')
        // BUG: this test fails because regex in HTML code supports underscores
    })
